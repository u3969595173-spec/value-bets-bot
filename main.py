...
# Deportes a monitorear (OPTIMIZADO: deportes principales con mercados expandidos)
SPORTS = os.getenv("SPORTS", "basketball_nba,basketball_euroleague,soccer_epl,soccer_spain_la_liga,soccer_germany_bundesliga,soccer_italy_serie_a,soccer_france_ligue_one,soccer_uefa_champs_league,soccer_uefa_europa_league,soccer_uefa_europa_conference_league,americanfootball_nfl,tennis_atp,tennis_wta").split(",")
...
"""
    ...
    async def handle_button_message(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        # Stats
        if text == "📊 Mis Stats":
            # Llamar directamente a cmd_stats_pro
            from commands.verification_commands import cmd_stats_pro
...
"""
    ...
    def setup_telegram_handlers(self):
        ...
        try:
            ...
            logger.info("✅ Registrando /limpiar_pendientes...")
            self.telegram_app.add_handler(CommandHandler("limpiar_pendientes", cmd_limpiar_pendientes))
            self.telegram_app.add_handler(CommandHandler("limpiar", cmd_limpiar_pendientes))  # Alias corto
